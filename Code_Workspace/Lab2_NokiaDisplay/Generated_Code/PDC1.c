...
/* For Bender Splash Screen */
extern const unsigned char bender [];
...
/*
** ===================================================================
**     Method      :  benderSplash(void)
**     Description :
**         Displays Futureramas' Bender to the screen
**     Parameters  : None
**     Returns     : Nothing
** ===================================================================
*/
void benderSplash(void)
{
    int counter = 0;
	// int benderptr = *bender;
	PDC1_Clear();

	for(PDC1_PixelDim i=0; i<84; i++)
	{
		for(PDC1_PixelDim j=0; j<6; j++)
		{
			//PDC1_SetPos(i,j);
			WriteData((byte)bender[counter]);
			counter ++;
		}
	}
}
...
/*
** ===================================================================
**     Method      :  WriteChar4X(char inputchar, int x, int y)
**     Description :
**         Writes characters 4x the size
**     Parameters  :
**     		inputchar
**     		x
**     		y
...
** ===================================================================
*/
void WriteChar4X(char inputchar, int x, int y)
{
	unsigned char oldchar_bytes[WIDTH];
	unsigned char newchar_bytes[WIDTH*4];
	unsigned char tempbyte;
	int i,offset,mask;
	// Extract relevant bytes from table
	for(i=0;i<WIDTH;i++)
	{
		oldchar_bytes[i] = table[(PDC1_FONT_COLUMNS_PER_CHAR*((int)inputchar))-160+i];
	}
	// Double the width of upper part
	for(i=0;i<WIDTH*2;i++)
	{
		newchar_bytes[i] = oldchar_bytes[i/2]<<4;
	}
	// Shift to top
	for(i=0;i<WIDTH*2;i++)
	{
		newchar_bytes[i] = newchar_bytes[i]>>4;
	}
	// Stretch top half
	for(i=0;i<WIDTH*2;i++)
	{
		// Iterate through each bit of each vertical byte
		// and set the temporary byte accordingly
		tempbyte = 0; // initialize the temporary byte
		for(mask=0; mask<8; mask++)
		{
			if(newchar_bytes[i] & (0x01<<mask))
			{
				// set proper bits in temporary byte
				tempbyte |= (0x01<<(mask*2));
				tempbyte |= (0x01<<(mask*2+1));
			}
			else
			{
				// clear proper bits in temporary byte
				tempbyte &= ~(0x01<<(mask*2));
				tempbyte &= ~(0x01<<(mask*2+1));
			}

		}
		// set temporary byte to the current byte in the
		// new character array
		newchar_bytes[i] = tempbyte;
	}

	// Double the width of the lower part
	// and shift
	for(i=WIDTH*2;i<WIDTH*4;i++)
	{
		newchar_bytes[i] = (oldchar_bytes[(i-(WIDTH*2))/2]&0xF0)>>4;
	}
	// Stretch the bottom half
	for(i=WIDTH*2;i<WIDTH*4;i++)
	{
		tempbyte = 0;
		for(mask=0; mask<8; mask++)
	...
	// Set cursor position to top, end of character
	PDC1_SetPos(x+WIDTH+1,y+1);
}
