...
byte SD1_Init(void* unused)
{
  ...
  WAIT2_Waitms(1);                      /* wait at least for 1 ms on insertion and power on */
  ...
  WAIT2_Waitus(50);                     /* need to wait a little bin in order SPI transfer to be finished (need this on CN128, but on others too?) */
  SD1_Deactivate();
  /* -------------------------------- IDLE Command -------------------------------- */
  arg = 0;
