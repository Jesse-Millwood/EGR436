...
**     Date/Time   : 2015-02-02, 18:19, # CodeGen: 24
**     Abstract    :
**
**     Settings    :
**
...
extern const unsigned char Night [];
...
{
	if(0<=x<PDC1_DISPLAY_HW_NOF_COLUMNS)
	{
		WriteCmd((byte)(0x40|(y&0x07)));      /* Y axis */
		WriteCmd((byte)(0x80|x)); /* X axis */
	}
...
// Functions for Weather Logger Display
void PDC1_BenderScreen(void)
{
    uint16_t counter = 0;
	PDC1_Clear();
...
void PDC1_SplashScreen(void)
{
	...
	for(PDC1_PixelDim i=0; i<84; i++)
	{
		for(PDC1_PixelDim j=0; j<6; j++)
		{
			WriteData((byte)Splash[counter]);
			counter ++;
		}
	}
}

void PDC1_Cloud(uint8_t x, uint8_t y)
{
	uint16_t counter = 0;

	for(PDC1_PixelDim iy=0; iy<=1; iy++)
	{
		PDC1_SetPos(x,y+iy);
		for(PDC1_PixelDim ix=0; ix<20; ix++)
		{
			//WriteData((byte)0xFF);
			WriteData((byte)Cloud[counter]);
			counter++;
		}
	}
}

void PDC1_Sun(uint8_t x, uint8_t y)
{
	uint16_t counter = 0;
	...
	for(PDC1_PixelDim iy=0; iy<=1; iy++)
	{
		...
		{
			//WriteData((byte)0xFF);
			WriteData((byte)Sun[counter]);
			counter++;
		}
	}
}

void PDC1_Moon(uint8_t x, uint8_t y)
{
	uint16_t counter = 0;
